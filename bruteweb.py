...
MAX_THREADS = 20
TIMEOUT = 5
MAX_LENGTH = 2  # configurable via CLI
        ...
        for combo in itertools.product(CHARSET, repeat=length):
            ...
            for ext in COMMON_EXTENSIONS:
                if args.verbose == True:
                    print(f"[i] Trying: {base+ext}")
                yield base + ext


def is_wildcard_response(base_url, test_string="nonexistent_path_123abcxyz"):
...
def brute_force(base_url, max_length):
    ...
    if args.verbose == False:
        with tqdm(desc="Bruteforcing...", unit="req", leave=True, colour="Blue") as pbar:
            ...
            # Feed generator into queue live
        ...
        threads = []
        for _ in range(MAX_THREADS):
            t = threading.Thread(target=worker, args=(base_url, wildcard_status, wildcard_content))
            t.daemon = True
            t.start()
            threads.append(t)
...
print("[i] Let the games BEGIN!")

if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(prog="WB IT!", description="Webserver Bruteforcer Internet Tool", epilog="Generic usage: python name.py https://example.com/ --maxlen 2")
    ...
    parser.add_argument("--verbose", type=bool, default=False, help="Show what it is trying to crack - but hide progressbar (default: False)")
